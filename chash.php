...
if (!$config_file = _chash_find_config_file()) {
  die(_t("Couldn't find config file. Please either give the path to the Chamilo installation you want to act on, through the -c option, or 'cd' into a valid Chamilo installation directory"));
}
/**
...
function _chash_get_all_databases() {
  ...
  if (!in_array($_configuration['user_personal_database'],$dbs) && !empty($_configuration['user_personal_database'])) {
...
function chash_command_full_backup($params) {
  ...
  if (empty($params)) {
    echo _t('No parameters provided.')."\n";
    echo _t('The full_backup command allows you to do a full backup of the files and database of a given Chamilo installation')."\n";
    ...
    echo '  --tmp'."\t"._t('Allows you to specify in which temporary directory the backup files should be placed (optional, defaults to /tmp)')."\n";
    echo '  --del-archive'."\t"._t('Deletes the contents of the archive/ directory before the backup is executed')."\n";
    return false;
  }
...
function chash_command_clean_archives() {
  ...
  if (empty($_configuration['root_sys'])) {
    echo _t('$_configuration[\'root_sys\'] is empty. In these conditions, it is too dangerous to proceed with the deletion. Please ensure this variable is defined in main/inc/conf/configuration.php')."\n";
    return false;
  }
